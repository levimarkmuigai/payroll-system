 ...
 * Provides endpoints to create, read, update, and delete NSSF configuration
 * data.
 */
@RestController
...
public class NSSFConfigController {
     ...
     * @return the updated NSSFConfig wrapped in a ResponseEntity, or 404 if not
     *         found.
     */
    @PutMapping("/{id}")
    public ResponseEntity<NSSFConfig> updateNSSFConfig(@PathVariable Long id,
            @RequestBody NSSFConfig updatedNSSFConfig) {
        return nssfConfigRepository.findById(id)
                .map(existingConfig -> {
